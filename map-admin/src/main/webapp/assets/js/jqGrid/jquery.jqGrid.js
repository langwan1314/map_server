...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		ynf_extend_args = function(old_args,new_args){ //导航参数
			$.extend(old_args , new_args) ; 
			return old_args ;
		},
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
                ...
                var ynf_table_nav_args = $(ts).data("ynf_table_nav_args") || {};

				if(ynf_table_nav_args._search === false){
					delete ts.p.postData.filters ;
				}

				// debugger ;

                console.log("YNF_CONFIG.ynf_table_nav",ynf_table_nav_args  , $(ts) );

				ts.p.postData  = ynf_extend_args(ts.p.postData , ynf_table_nav_args ); //dev
				// YNF_CONFIG.ynf_table_nav = {}; //将ynf_table_nav 释放 ；
				delete ynf_table_nav_args.page ;
				$(ts).data("ynf_table_nav_args",ynf_table_nav_args);

                // delete YNF_CONFIG.ynf_table_nav.page ;
				...
				case "script":
                 ...
                 // console.log( "$.jgrid.ajaxOptions" ,  $.jgrid.ajaxOptions  ); //dev 
                 // console.log( "ts.p.ajaxGridOptions" ,  ts.p.ajaxGridOptions  ); //dev 
...
"use strict";	
	...
	$.fn.fmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton && !op.edit_is_link){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'formedit'); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='c-blue' style='font-size:16px;'>编辑</span></div>";
		} else if(op.editbutton &&  !op.edit_is_link){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'edit'); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover') ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='c-blue'  style='font-size:16px;' >编辑</span></div>";
		} else if( op.edit_is_link && op.edit_link ){ //dev change
			ocl = "href='"+op.edit_link+rowid+"' style='text-decoration: none;'  onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover') ";
			str += "<a title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='c-blue'  style='font-size:16px;'>编辑</span></a>";
		}
		if(op.delbutton) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.ynf_del) {
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							...
							if(dr){
								if($.isFunction( o.ynf_delfunc )){
									console.log("执行------","o.ynf_delfunc");
									o.ynf_delfunc.call($t, dr);
								}else{
									$($t).jqGrid("delGridRow",dr,pDel);
								}
							} else  {
			   ...
			   if (o.ynf_toper) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pDel = pDel || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.ynf_topericon+"'></span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.ynf_topertitle || "",id: pDel.id || "top_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
							...
							if(dr){
								if($.isFunction( o.ynf_toperfunc )){
									console.log("执行------","o.ynf_toper");
									o.ynf_toperfunc.call($t, dr);
								}else{
									$($t).jqGrid("delGridRow",dr,pDel);
								}
							} else  {
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			if (o.url === 'clientArray') {
				...
				var data = $.isFunction($t.p.serializeRowData) ? $t.p.serializeRowData.call($t, tmp3) : tmp3;
				if(window._table_update_ && $.isPlainObject(window._table_update_ ) ){
					for(var i in data){

                     if( $.isFunction( window._table_update_[i] ) && window._table_update_[i](data)  === false)  return ;
					}
				}
				$.ajax($.extend({
					url:o.url,
					...
					error:function(res,stat,err){
						$("#lui_"+$.jgrid.jqID($t.p.id)).hide();
						$.ynf.small_modal({show:true,body_cont : "更新成功！"});
