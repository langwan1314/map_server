...
/**
 * @Description: 实现自己的日志适配 按时间日期每天生成一份日志，且当日志文件大小超过设定，自动再拆分出日志文件
 */
public class DailyRollingFileAppender extends FileAppender {

    //用于处理防止在debug日志中记录info类型的日志
     ...
     */
    //当MaxBackupIndex为-1时候 无限的文件数量，index顺序按时间顺序递增
    ...
    void sizeRollOver() {
        ...
        if (maxBackupIndex > 0) {
            ...
            file = new File(datedFilename + '.' + maxBackupIndex+".txt");
            if (file.exists())
                file.delete();
            ...
            for (int i = maxBackupIndex - 1; i >= 1; i--) {
                file = new File(datedFilename + "." + i+".txt");
                if (file.exists()) {
                    target = new File(datedFilename + '.' + (i + 1)+".txt");
            ...
            target = new File(datedFilename + "." + 1+".txt");
            ...
            for (int i = 1; i < Integer.MAX_VALUE; i++) {
                target = new File(datedFilename + "." + i+".txt");
...
class RollingCalendar extends GregorianCalendar {
   ...
   private static final long serialVersionUID = -6543822254510319307L;
